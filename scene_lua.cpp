...
int gr_keyframe_cmd(lua_State* L)
{
  ...
  uint easing = luaL_checknumber(L, 4);

  data->node = new KeyframeNode(name, startTime, endTime, easing);
...
int gr_arealight_cmd(lua_State* L)
{
  ...
  get_tuple(L, 4, &al.u[0], 3);
  get_tuple(L, 5, &al.v[0], 3);

  al.colour = glm::vec3(col[0], col[1], col[2]);
  
  data->light = new AreaLight(al);
...
int gr_render_cmd(lua_State* L)
{
  ...
  gr_node_ud* root = (gr_node_ud*)luaL_checkudata(L, ++arg, "gr.node");
  luaL_argcheck(L, root != 0, arg, "Root node expected");

  const char* filename = luaL_checkstring(L, ++arg);

  int width = luaL_checknumber(L, ++arg);
  int height = luaL_checknumber(L, ++arg);
  
  int time = 12;
  if (lua_type(L, arg + 1) == LUA_TNUMBER)
    time = luaL_checknumber(L, ++arg);

  glm::vec3 eye;
  glm::vec3 view, up;
  
  get_tuple(L, ++arg, &eye[0], 3);
  get_tuple(L, ++arg, &view[0], 3);
  get_tuple(L, ++arg, &up[0], 3);

  double fov = luaL_checknumber(L, ++arg);

  double ambient_data[3];
  get_tuple(L, ++arg, ambient_data, 3);
  glm::vec3 ambient(ambient_data[0], ambient_data[1], ambient_data[2]);

  luaL_checktype(L, ++arg, LUA_TTABLE);
  int light_count = int(lua_rawlen(L, arg));
  
  luaL_argcheck(L, light_count >= 0, arg, "Tuple of lights expected");
  std::list<Light*> lights;
  for (int i = 1; i <= light_count; i++) {
    lua_rawgeti(L, arg, i);
    gr_light_ud* ldata = (gr_light_ud*)luaL_checkudata(L, -1, "gr.light");
    luaL_argcheck(L, ldata != 0, arg, "Light expected");

    lights.push_back(ldata->light);
    lua_pop(L, 1);
  }

  double focalDistance = luaL_optnumber(L, ++arg, 1);
  double blurAmount = luaL_optnumber(L, ++arg, 0);

	Image im( width, height);
	A5_Raytracer raytracer(root->node, eye, view, up, fov, ambient, lights, focalDistance, blurAmount);
  int runtime = raytracer.render(im, time);
  im.savePng( filename );
  std::cout << "Rendered frame in " << runtime << " ms" << std::endl;

	return 0;
}

// Render an animation of a scene
extern "C"
int gr_animate_cmd(lua_State* L)
{
  ...
  const char* filenameFmt = luaL_checkstring(L, ++arg);

  int width = luaL_checknumber(L, ++arg);
  int height = luaL_checknumber(L, ++arg);
  
  int numFrames = luaL_checknumber(L, ++arg);
  double startTime = luaL_checknumber(L, ++arg);
  double timeIncrement = luaL_checknumber(L, ++arg);
  ...
  int parallelIndex = luaL_optnumber(L, ++arg, 0);
  int parallelTotal = luaL_optnumber(L, ++arg, 1);
  ...
  for (int i = parallelIndex; i < numFrames; i += parallelTotal)
  {
    sprintf(filename, filenameFmt, i);
    time = startTime + i * timeIncrement;
    int runtime = raytracer.render(im, time);
    im.savePng(filename);
    std::cout << "Rendered frame " << filename << " (t=" << time << ") in " << runtime << " ms" << std::endl;
  }
...
int gr_material_cmd(lua_State* L)
{
  ...
  double reflectivity = luaL_optnumber(L, 4, 0);
  double transparency = luaL_optnumber(L, 5, 0);
  double refractiveIndex = luaL_optnumber(L, 6, 1);
  double glossiness = luaL_optnumber(L, 7, 0);
